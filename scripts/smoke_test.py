...
def main() -> int:
    ...
    print("\n== 2. read real data + Arrow pandas_udf ==")
    csv = os.path.join(REPO, "data", "flights.csv")
    df = spark.read.csv(csv, header=True, inferSchema=False).limit(1000)
    n = df.count()
    results.append(_check("read 1000 rows of flights.csv", n == 1000, f"got {n}"))
    ...
    tmp = tempfile.mkdtemp(prefix="smoke_model_", dir=os.path.join(REPO, ".spark-tmp"))
