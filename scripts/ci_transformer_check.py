...
def main() -> int:
    ...
    try:
        ...
        check("OutlierIQRTruncator directly subclasses Transformer",
              issubclass(OutlierIQRTruncator, Transformer))
        direct = OutlierIQRTruncator(
            inputCols=["DEPARTURE_DELAY"], outputCols=["direct_clip"])
        direct_max = direct.transform(train).agg(F.max("direct_clip")).first()[0]
        raw_max = train.agg(F.max("DEPARTURE_DELAY")).first()[0]
        check("direct Transformer computes Q1/Q3 and clips standalone",
              direct_max < raw_max, f"raw max={raw_max:.1f} clipped={direct_max:.1f}")
        ...
        iqr = OutlierIQRTruncatorEstimator(inputCols=["DEPARTURE_DELAY"],
                                  outputCols=["dep_clip"])
        ...
        pipe = Pipeline(stages=[
            ...
            OutlierIQRTruncatorEstimator(inputCols=["DEPARTURE_DELAY"], outputCols=["dep_clip"]),
