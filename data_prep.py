...
from spark_session import build_spark, path, raw_data_path  # noqa: E402
...
def step_raw(spark) -> None:
    flights_csv = raw_data_path("flights.csv")
    print(f"reading {flights_csv}")
    df = spark.read.csv(flights_csv, header=True,
...
def step_curate(spark) -> None:
    ...
    airports = spark.read.csv(raw_data_path("airports.csv"), header=True, inferSchema=True)
    airlines = spark.read.csv(raw_data_path("airlines.csv"), header=True, inferSchema=True)
...
def main() -> None:
    ...
    ap.add_argument("--raw-dir", help="directory containing flights/airports/airlines CSVs")
    args = ap.parse_args()

    if args.raw_dir:
        os.environ["FLIGHT_RAW_DIR"] = os.path.abspath(args.raw_dir)
