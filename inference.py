...
def main() -> None:
    ...
    if args.from_registry:
        ...
        # MLflow 2 uses stages; MLflow 3 replaced them with aliases. Prefer the
        # modern URI and fall back so one inference script serves both.
        uris = ["models:/flight_delay_pipeline@Production",
                "models:/flight_delay_pipeline/Production"]
        for uri in uris:
            try:
                print(f"loading {uri}")
    ...
    try:
        if args.await_seconds:
            ...
            # A timeout can expire while an Arrow UDF micro-batch is still in
            # flight (especially on a cold Windows JVM). Finish all files that
            # were already visible before stopping, otherwise Spark aborts the
            # parquet commit and the demo reports zero rows despite valid input.
            sink.processAllAvailable()
            console.processAllAvailable()
        else:
