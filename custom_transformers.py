...
Pipeline stages
    ...
    ``OutlierIQRTruncator``       direct Transformer with frozen Tukey fences
    ``OutlierIQRTruncatorEstimator`` learns those fences on training data
...
**How the direct IQR Transformer remains leak-safe.**
...
parameters, so they belong to a fit stage: ``OutlierIQRTruncatorEstimator``
computes them on the training split only and freezes them into the direct
``Transformer`` named ``OutlierIQRTruncator``. That satisfies the assignment's
literal inheritance requirement without leaking held-out data. It is what makes the
...
class OutlierIQRTruncator(Transformer, HasInputCols, HasOutputCols, _IQRParams,
                          DefaultParamsReadable, DefaultParamsWritable):
    """Direct Transformer that clips columns to pre-fitted Tukey fences.

    With fitted bounds it performs only native clipping. For the assignment's
    literal standalone use it can compute Q1/Q3 when bounds are omitted. Use
    :class:`OutlierIQRTruncatorEstimator` inside a training Pipeline so every
    cross-validation fold learns its own train-only fences.
    """
    ...
    def _transform(self, dataset: DataFrame) -> DataFrame:
        if self.isSet(self.lowerBounds) and self.isSet(self.upperBounds):
            lo = self.getOrDefault(self.lowerBounds)
            hi = self.getOrDefault(self.upperBounds)
        else:
            # Literal standalone Transformer behaviour requested by the brief:
            # compute distributed Q1/Q3 when no fitted fences were supplied.
            # The production/CV Pipeline never takes this branch; its preceding
            # Estimator supplies train-only bounds to keep evaluation leak-safe.
            cols = self.getInputCols()
            eps = float(self.getOrDefault(self.relativeError))
            k = float(self.getOrDefault(self.iqrMultiplier))
            casted = dataset.select(
                [F.col(c).cast("double").alias(c) for c in cols])
            quantiles = casted.approxQuantile(cols, [0.25, 0.75], eps)
            lo, hi = [], []
            for q1, q3 in quantiles:
                iqr = q3 - q1
                lo.append(float(q1 - k * iqr))
                hi.append(float(q3 + k * iqr))
...
class OutlierIQRTruncatorModel(OutlierIQRTruncator):
    """Backward-compatible loader for PipelineModels saved before the rename."""


class OutlierIQRTruncatorEstimator(Estimator, HasInputCols, HasOutputCols, _IQRParams,
                                   DefaultParamsReadable, DefaultParamsWritable):
    ...
    def _fit(self, dataset: DataFrame) -> OutlierIQRTruncator:
        cols = self.getInputCols()
        k = float(self.getOrDefault(self.iqrMultiplier))
        eps = float(self.getOrDefault(self.relativeError))
