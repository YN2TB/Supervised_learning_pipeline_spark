...
full feature set against $0.703$ through `PCA(k=10)`.
...
### B2.1 Direct `OutlierIQRTruncator` Transformer without leakage

The brief explicitly requires `OutlierIQRTruncator` to subclass
`pyspark.ml.Transformer`, and it does. Used standalone without bounds, it computes
distributed $Q_1,Q_3$ and clips exactly as requested. Used in the production
Pipeline, its fitted bounds are frozen Params and `_transform` applies only native
Catalyst expressions to the DataFrame being scored.

Clipping fences are **learned parameters**, so the training Pipeline contains
`OutlierIQRTruncatorEstimator`. Its `_fit` computes $Q_1,Q_3$ on the training split
and returns the direct Transformer `OutlierIQRTruncator`. This satisfies the
literal inheritance requirement while keeping test and streaming data out of the
fit. Phase 2 verifies that `transform(test)` respects the train-only fences.
...
```
OutlierIQRTruncatorEstimator → OutlierIQRTruncator (direct Transformer)
...
cross-validation runs automatically. Evaluation metrics and JSON, cumulative
explained-variance JSON and PNG, ranked feature-importance metrics, feature names,
JSON and PNG are logged explicitly. The winning regression pipeline is registered
and promoted **Staging → Production**.

The pinned MLflow 2.x environment uses the stage API named by the brief. The same
code also supports MLflow 3.x by publishing `Staging` and `Production` aliases and
matching `deployment_stage` model-version tags.
